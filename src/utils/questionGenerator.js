...
function applyNoAnswerPenalty(result, keyPoints) {
  return {
    ...
    consistency: "pass",
    consistencyScore: 1,
    consistencyReason: "คำตอบไม่สมบูรณ์ (ปฏิเสธ/ไม่พบข้อมูล) ไม่ถือว่าผ่านแม้ไม่มีข้อความขัดแย้ง — ความสอดคล้องภายใน 1.0",
  };
}
...
- คะแนนเริ่มต้น = จำนวนประเด็นที่ครอบคลุม / จำนวนประเด็นทั้งหมด
- ตัวอย่าง: 3 ประเด็น ตอบครบแค่ 1 ประเด็น → ประมาณ 0.33
- ข้อมูลขัดแย้งกับแนวทางชัดเจน → ลดคะแนน

ปรับลดเพิ่มเติมตามระยะห่างจากประเด็นคำถาม (relevance penalty):
- ตอบตรงประเด็น เนื้อหาเกี่ยวข้องโดยตรง → ไม่ปรับลด
- ตอบนอกประเด็นบางส่วน เช่น ผิด scope (ตอบภาพรวมแทนรายสถานี, ผิดจังหวัด, ผิดโครงการ) → หาร 2 จากคะแนนที่คำนวณได้
- ตอบผิดประเด็นอย่างชัดเจน เนื้อหาไม่เกี่ยวกับสิ่งที่ถามเลย → accuracy_score ไม่เกิน 0.1

- สำคัญ: ถ้าตอบแค่ "ไม่พบข้อมูล/ไม่มีในฐานข้อมูล" โดยไม่ให้เนื้อหา → accuracy_score=0, consistency=pass (ไม่มีข้อความขัดแย้งภายใน)
...
- สำคัญ (timeframe): ถ้าคำถามถามข้อมูล "สัปดาห์นี้/วันนี้/ล่าสุด" แต่คำตอบให้เฉพาะค่าเฉลี่ยระยะยาว (เช่น เฉลี่ย 67 สัปดาห์) โดยไม่มีค่าของช่วงเวลาที่ถาม → ถือว่า missed keyPoint นั้น แม้จะมีตัวเลขก็ตาม
    ...
    if (isClarificationOnlyResponse(answer, question)) {
      ...
      if (kw.score > 0) {
        ...
        result = attachJudgeMeta({
          ...
          consistency: 'pass',
          consistencyScore: hasData ? 0.5 : 1,
          consistencyReason: hasData
            ? 'อธิบายข้อจำกัดข้อมูล แล้วให้ข้อมูลที่มีแทน — สอดคล้องภายใน (0.5)'
            : 'คำตอบไม่สมบูรณ์ (ปฏิเสธ/ไม่พบข้อมูล) ไม่ถือว่าผ่านแม้ไม่มีข้อความขัดแย้ง — ความสอดคล้องภายใน 1.0',
        }, judgeModel)
      } else {
